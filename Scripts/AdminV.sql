...
Majd jelöljük ki a szükséges táblákat, majd Save to után előáll a script (vagy új Query ablakban, vagy a Clipboard-on)
-------------------------------*/
...
-- A meglévő táblák törlése megfelelő sorrendben
...
-- Az AddExtendedProperty sorokat ki lehet törölni a generált script-ből.
...
BACKUP DATABASE [Vendors] TO  DISK = N'C:\VendorDB\Backup\VendorFullBackup.bak' WITH NOFORMAT, NOINIT,  NAME = N'Vendors-Full Database Backup', SKIP, NOREWIND, NOUNLOAD,  STATS = 10, CHECKSUM
GO
...
BACKUP DATABASE [Vendors] TO  DISK = N'C:\VendorDB\Backup\VendorsDiffBackup.sql' WITH  DIFFERENTIAL , NOFORMAT, NOINIT,  NAME = N'Vendors-Full Database Backup', SKIP, NOREWIND, NOUNLOAD,  STATS = 10, CHECKSUM
GO
declare @backupSetId as int
select @backupSetId = position from msdb..backupset where database_name=N'Vendors' and backup_set_id=(select max(backup_set_id) from msdb..backupset where database_name=N'Vendors' )
if @backupSetId is null begin raiserror(N'Verify failed. Backup information for database ''Vendors'' not found.', 16, 1) end
RESTORE VERIFYONLY FROM  DISK = N'C:\VendorDB\Backup\VendorsDiffBackup.sql' WITH  FILE = @backupSetId,  NOUNLOAD,  NOREWIND
GO
